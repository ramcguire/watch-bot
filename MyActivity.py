...
#since there can be multiple activities per member, each MyActivity should contain timestamp and timespent
...
class MyGame():
    ...
    def update_start_time(self, start_time):
        if self.in_game:
            logging.warning('found already in_game while trying to update_start_time')
    ...
    def update_timestamps(self):
        ...
        if self.start_time is None or self.end_time is None:
            ...
            print('invalid update_timestamps in MyGame, start or end time is None')
            return
    ...
    def get_total_time_in_game(self):
        ...
        if self.in_game:
            time += round((pendulum.now('UTC') - self.start_time).total_seconds())
        return time
